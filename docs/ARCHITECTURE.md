...
A User can access multiple Tenants with one or two roles per Tenant Membership. Administrators manage all Users and Invitations in their Tenant; Dispatchers manage operational Driver assignments, not User accounts. The Administrator-only UsersManage endpoint policy authorizes User Management. Handlers use IUserContext to scope target records to the selected Tenant and identify the actor without resolving or re-authorizing the manager. These records cannot use ambient Tenant filtering because invitation acceptance and identity resolution also run before Tenant selection. PostgreSQL enforces a unique User/Tenant membership pair and a unique pending invitation per Tenant/email pair. Roles, active status, optimistic concurrency, and access history belong to Tenant Memberships; the User identity and profile are shared.

Invitations are committed before Keycloak sends the organization signup email; delivery failures remain visible and can be retried explicitly. Acceptance checks the live Keycloak identity's verified email and Organization membership, then atomically creates or reuses the User and adds the Tenant Membership. Existing users may list and accept additional invitations without changing the manager's current Tenant or selecting a Tenant on the invite form. Expired or revoked invitations grant no MDSweep access, including when an old Keycloak link still exists. Deactivation is enforced per membership from the database on every protected request; other Tenant memberships retain their access. Access history retains changes without deleting Users. SMTP configuration remains deferred; see [User management](./user-management.md).
