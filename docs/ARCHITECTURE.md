...
Invitations are committed before the invitation email is sent; delivery failures remain visible and can be retried explicitly. Each invitation stores only a hash of a cryptographically random token. The secure link supplies the raw token to the authenticated acceptance endpoint, so the invitation—not the client—determines the Tenant, email, roles, and expiry. Acceptance checks the authenticated Keycloak identity's verified email, then atomically creates or reuses the local User and adds the Tenant Membership. It does not require an already selected Tenant. Resending rotates the token, and expired, revoked, accepted, or superseded tokens grant no MDSweep access. Deactivation is enforced per membership from the database on every protected request; other Tenant memberships retain their access. User and Invitation auditing and production SMTP configuration remain deferred; see [User management](./user-management.md).
