...
Each User has one Tenant and one or two roles. Administrators manage all Users and Invitations in their Tenant; Dispatchers manage Users whose only role is Driver. User-management handlers use the existing repository and a BFF-supplied actor context to authorize operations on the global Access records, including target roles. These records cannot use ambient Tenant filtering because invitation acceptance and identity resolution also run before Tenant selection. A unique membership per User and a composite User/Tenant foreign key enforce the single-Tenant boundary in PostgreSQL.

Invitations are committed before Keycloak sends the organization signup email; delivery failures remain visible and can be retried explicitly. Acceptance checks the live Keycloak identity's verified email and Organization membership, then atomically creates the User and Tenant Membership. Expired or revoked invitations grant no MDSweep access, including when an old Keycloak link still exists. Deactivation is enforced from the database on every protected request. Access history retains changes without deleting Users. SMTP configuration remains deferred; see [User management](./user-management.md).
