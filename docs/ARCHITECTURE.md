...
Trip Import is a one-step Trips operation: the Dispatcher submits a broker-provided CSV/XLSX file and receives its outcome. No preview, import identifier, or retained source-row lifecycle exists. The complete MTM Trip Number is the tenant-scoped external Trip identity. Re-importing creates, updates broker-owned data, or leaves a Trip unchanged without replacing tenant operational data; a missing row never implies deletion.
...
A User can access multiple Tenants with one or two roles per Tenant Membership. Administrators manage all Users and Invitations in their Tenant; Dispatchers manage operational Driver assignments, not User accounts. The Administrator-only UsersManage endpoint policy authorizes User Management. Handlers use IUserContext to scope target records to the selected Tenant and identify the actor without resolving or re-authorizing the manager. These records cannot use ambient Tenant filtering because invitation acceptance and identity resolution also run before Tenant selection. PostgreSQL enforces a unique User/Tenant membership pair and a unique pending invitation per Tenant/email pair. Roles, active status, and optimistic concurrency belong to Tenant Memberships; the User identity and profile are shared.

Invitations are committed before Keycloak sends the organization signup email; delivery failures remain visible and can be retried explicitly. Each invitation stores only a hash of a cryptographically random token. The secure link supplies the raw token to the authenticated acceptance endpoint, so the invitation—not the client—determines the Tenant, email, roles, and expiry. Acceptance checks the live Keycloak identity's verified email and Organization membership, then atomically creates or reuses the User and adds the Tenant Membership. It does not require an already selected Tenant. Resending rotates the token, and expired, revoked, accepted, or superseded tokens grant no MDSweep access. Deactivation is enforced per membership from the database on every protected request; other Tenant memberships retain their access. User and Invitation auditing and SMTP configuration remain deferred; see [User management](./user-management.md).

ASP.NET Core is the OpenID Connect client and backend-for-frontend: it establishes the HttpOnly application cookie after authorization code authentication with Keycloak. Angular calls same-origin application endpoints and never receives or manages Keycloak tokens. The API maps Keycloak's immutable `sub` to the local User ID, returns the active Tenant, allowed memberships, and an antiforgery cookie from `/api/auth/session`, and accepts a membership-verified selection at `/api/auth/tenant-context`. The selected Tenant ID is stored in the signed application cookie as `mdsweep_tenant_id`; Wolverine detects it globally for conjoined tenancy. Reusable authorization policies verify the User's membership and role for protected resources. The application never trusts a client-supplied Tenant ID.
...
PostgreSQL hosts one MDSweep application database and the existing separate Keycloak database. MDSweep uses checked-in EF Core migrations for application schema, with `InitialSchema` serving as a reset pre-production baseline rather than an upgrade from the removed chain. `UserManagementAndInvitations` creates User management with multiple-Tenant memberships, membership-owned active flags and versions. `UserRoles` adds role arrays without changing membership ownership. Tests cover the baseline upgrade with memberships in multiple Tenants and preserve access and concurrency versions across the role upgrade. A downgrade cannot discard a second role. Wolverine uses its own persistence schema for its conjoined-tenant registry and message persistence. Use EF Core mappings near the owning feature. Preserve broker-provided details separately from Tenant-owned changes and retain append-only history for Manifest receipts, Assignments, Performed Vehicle snapshots, actual timestamps, outcomes, corrections, and closure.
...
Wolverine's lightweight EF Core transaction middleware calls one `SaveChangesAsync` at the end of a successful EF-backed handler. `AutoApplyTransactions()` infers this from `IRepository`; commands and queries do not use Wolverine transaction attributes. Invitation creation and email delivery are separate commands so the invitation survives a delivery failure. Acceptance uses a single local transaction and does not create or delete Keycloak identities.
