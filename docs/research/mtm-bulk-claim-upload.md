...
| `Vehicle` | The standalone Vehicles module maintains Tenant-owned display labels, required VINs, and active state | **Trip integration remains missing.** Add effective-dated Driver Primary Vehicle designations, resolve the Performed Vehicle automatically, allow Dispatcher exceptions, and preserve its VIN as historical Trip data. Vehicle record management alone does not establish billing readiness or verify MTM registration. See [Vehicle management](../vehicle-management.md). |
