...
- A User has one Tenant Membership and one or two distinct roles chosen from Administrator, Dispatcher, and Driver. Administrators manage all Users; Dispatchers manage Users whose only role is Driver. The recipient accepts an invitation before gaining access. A Dispatcher completes the Driver Profile separately after acceptance, before Trip Assignment.
