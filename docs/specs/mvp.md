...
- Build a modular monolith with deep Passengers and Trips modules plus separate Vehicles and Access modules. Passenger and Vehicle record management are independent of Trip lifecycle; organize Manifest intake, planning, performance, review, and billing as internal vertical slices of Trips rather than top-level domain modules.
...
- Tenant Memberships isolate access between transportation businesses. Users may access multiple Tenants and select their working Tenant after authentication. Tenant creation and administration remain separate from User invitations.
...
- A User may have memberships in multiple Tenants, each with one or more distinct roles chosen from Administrator, Dispatcher, and Driver. Administrators manage all Users; Dispatchers manage operational Driver assignments, not User accounts. The recipient accepts an invitation before gaining access. A Dispatcher completes the Driver Profile separately after acceptance, before Trip Assignment.
...
- The separate Vehicles module provides its own sidebar item and page for Administrators and Dispatchers. It supports listing, searching by display label or VIN, filtering by All/Active/Inactive, creation, editing, deactivation, and reactivation. Display label is required and limited to 100 characters. VIN is required, normalized to uppercase at the API boundary, and must contain exactly 17 ASCII letters/digits excluding I, O, and Q. VIN is unique per Tenant even when inactive. New records start active; permanent deletion, checksum validation, and MTM verification are excluded.
- Vehicle record management ships independently of the following Trip integration requirements. It does not remove or implement Primary Vehicle designations, Vehicle exceptions, Performed Vehicle snapshots, or billing. See [Vehicle management](../vehicle-management.md).
