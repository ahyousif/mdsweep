...
- A User may have memberships in multiple Tenants, each with one or two distinct roles chosen from Administrator, Dispatcher, and Driver. Administrators manage all Users; Dispatchers manage Users whose only role is Driver. The recipient accepts an invitation before gaining access. A Dispatcher completes the Driver Profile separately after acceptance, before Trip Assignment.
