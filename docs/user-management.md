...
Only an Administrator can open this page or manage access. A User can access multiple Tenants through separate Tenant Memberships, each with one or more distinct roles chosen from Administrator, Dispatcher, and Driver. The detail panel supports editing the selected Tenant Membership's display name and roles, disabling or re-enabling that membership, and resending or cancelling a pending Invitation. All three roles may be selected together; permission checks retain their existing behavior. Global User names and email are read-only here. Administrators cannot deactivate themselves or remove their own Administrator role.

Choose **Invite user** to invite an email address to the active Tenant with a first name, last name, and one or more roles. There is no Tenant selector in the form. A successful request closes the dialog without changing the current search or status filter. A failed request keeps the entered values visible and shows an actionable error.
