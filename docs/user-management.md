...
The recipient follows the secure invitation link, completes signup or signs in to their existing Keycloak identity, and submits the link's one-time token to MDSweep. MDSweep stores only the token hash; the invitation supplies its Tenant, invited email, roles, and expiry. Acceptance does not require a selected Tenant, and MDSweep verifies the authenticated identity's enabled, verified email and membership in the expected Keycloak Organization before granting access. Tokens cannot be reused, and Resend invalidates the previous token. There is no invitation inbox or authenticated-email discovery endpoint.
