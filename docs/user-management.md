...
Invitations store their recipient, intended roles, expiry, latest delivery result, and current status. User and Invitation auditing is deferred. A failed email leaves a pending invitation with an actionable error and **Resend**. Duplicate pending invitations and existing memberships are rejected within the current Tenant. Email matching is case-insensitive. The same email may receive invitations from multiple Tenants, and an existing User can accept another Tenant's invitation with the same identity. An inactive membership must be reactivated instead of reinvited. Invitations expire after seven days; a successful resend starts another seven-day period. Keycloak's email link may expire sooner according to its realm action-token setting; resend to obtain a fresh link.

The recipient follows the secure invitation link, completes signup or signs in to their existing Keycloak identity, and submits the link's one-time token to MDSweep. MDSweep stores only the token hash; the invitation supplies its Tenant, invited email, roles, and expiry. Acceptance does not require a selected Tenant. MDSweep verifies the authenticated identity's verified email, creates or reuses the local User, and creates the invited Tenant Membership before granting access. Tokens cannot be reused, and Resend invalidates the previous token. There is no invitation inbox or authenticated-email discovery endpoint.

**Tenant access** is available from the User menu and Driver landing screen. It lists existing active memberships; users with multiple memberships choose which Tenant to open. Switching reloads the application and other open MDSweep tabs so the previous Tenant's cached responses and forms are discarded consistently with the shared BFF cookie. The session endpoint returns the selected Tenant from the signed BFF cookie and revalidates active memberships on the server. No Driver Profile or Trip Assignment is created by acceptance. Driver Profile completion, including the MTM Driver Number, is a separate later workflow. Users cannot be assigned Trips before acceptance.

Revocation and expiry prevent MDSweep access even if an old Keycloak signup link remains usable. Deactivation blocks authorization for that Tenant on the next protected request, including existing application sessions, without deleting the User or Tenant Membership. Access in other Tenants is unchanged. Replaying an accepted invitation never restores deactivated access. Administrators cannot deactivate themselves or remove their own Administrator role. Updates reject stale versions instead of overwriting another manager's changes.

## Email setup is deferred

No production SMTP credentials or production realm email settings were added. Invitation and password-reset email failures remain visible until their respective email delivery is configured. See [Keycloak email configuration](https://www.keycloak.org/docs/latest/server_admin/#_email) when Keycloak password-reset email setup is resumed.
