...
internal sealed class ProviderMembershipConfiguration : IEntityTypeConfiguration<ProviderMembership>
{
    public void Configure(EntityTypeBuilder<ProviderMembership> entity)
    {
        ...
        entity.HasOne<Provider>().WithMany().HasForeignKey(x => x.ProviderId).OnDelete(DeleteBehavior.Restrict);
        entity.HasOne<AppUser>().WithMany().HasForeignKey(x => x.AppUserId).OnDelete(DeleteBehavior.Restrict);
    }
}
