...
public sealed class UpdateVehicleEndpoint
{
    ...
    public static Task<IResult> Put(Guid id, UpdateVehicleRequest request, IMessageBus bus, CancellationToken ct) =>
        VehicleWriteResult.Execute(async () => (await bus.SendAsync(request.ToCommand(id), ct)).ToEndpointResult());
}
