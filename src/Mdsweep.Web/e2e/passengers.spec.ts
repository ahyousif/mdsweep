...
for (const language of ['en', 'ar'] as const) {
  test(`Passenger inspector edits and disables with confirmation in ${language}`, async ({ page }) => {
    ...
    const close = inspector.getByRole('button', { name: language === 'en' ? 'Close passenger details' : 'إغلاق تفاصيل الراكب' });
    ...
    const status = await inspector.locator('header').getByText(language === 'en' ? 'Active' : 'نشط', { exact: true }).boundingBox();
    expect(memberId && status && Math.abs(memberId.y + memberId.height / 2 - status.y - status.height / 2)).toBeLessThan(8);
    ...
    const detailsHeading = inspector.getByRole('heading', { name: language === 'en' ? 'Details' : 'التفاصيل' });
    await expect(detailsHeading.locator('..').getByRole('button', { name: edit, exact: true })).toBeVisible();
    await expect(detailsHeading.locator('..').getByRole('button', { name: disableShort, exact: true })).toBeVisible();
    await expect(inspector.getByRole('button', { name: language === 'en' ? 'Passenger actions' : 'إجراءات الراكب' })).toHaveCount(0);
    await inspector.getByRole('button', { name: edit, exact: true }).click();
    await expect(inspector.getByRole('heading', { name: language === 'en' ? 'Edit passenger' : 'تعديل الراكب' })).toBeVisible();
    ...
    await expect(confirmation).toContainText(language === 'en' ? 'historical data will be retained' : 'سجله السابق');
    await confirmation.getByRole('button', { name: disable }).click();
    await expect(confirmation).toHaveCount(0);
    await expect(inspector.getByText(language === 'en' ? 'Disabled' : 'معطّل', { exact: true })).toBeVisible();
    expect(passenger.isActive).toBe(false);
    await inspector.getByRole('button', { name: edit, exact: true }).click();
    await inspector.getByLabel(notes).fill('Updated while disabled');
    await inspector.getByRole('button', { name: save }).click();
    ...
    await expect(inspector.getByText(language === 'en' ? 'Active' : 'نشط', { exact: true })).toBeVisible();
    await page.setViewportSize({ width: 390, height: 700 });
    await expect(close).toHaveCount(0);
    const backToList = inspector.getByRole('button', { name: language === 'en' ? 'Back to passengers' : 'العودة إلى الركاب' });
