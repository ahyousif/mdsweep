...
export class DispatchApi {
  ...
  getServiceDay(serviceDate: string): Promise<DispatchTrip[]> {
    return firstValueFrom(
      this.http.get<DispatchTrip[]>(`/api/service-days/${serviceDate}/trips`),
    );
  }

  getConflicts(): Promise<DriverSyncConflict[]> {
    return firstValueFrom(
      this.http.get<DriverSyncConflict[]>('/api/driver-work/conflicts'),
    );
  }
  ...
  setScheduledPickupTime(tripNumber: string, value: string): Promise<void> {
    return firstValueFrom(
      this.http.put<void>(
        `/api/trips/${encodeURIComponent(tripNumber)}/scheduled-pickup-time`,
