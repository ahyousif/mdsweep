...
import { DispatchApi, DispatchTrip } from './day-board.api';
import { ManifestImportApi, ManifestPreview, PreviewRow } from '../trip-import/trip-import.api';
import { dispatcherQueryKeys, dispatcherQueryOptions } from './day-board.queries';
...
@Component({
  ...
  templateUrl: './day-board-page.html',
})
export class DayBoardPage {
  ...
  protected readonly conflictsQuery = injectQuery(() => ({
    queryKey: dispatcherQueryKeys.conflicts(),
    queryFn: () => this.dispatch.getConflicts(),
  }));

  protected readonly serviceDayQuery = injectQuery(() =>
    dispatcherQueryOptions(this.dispatch, this.serviceDate()),
  );
  ...
  protected readonly previewMutation = injectMutation(() => ({
    ...
    onError: (error) => this.error.set(httpErrorMessage(error, 'Unable to check this Manifest.')),
  }));
  ...
  protected readonly applyMutation = injectMutation(() => ({
    ...
    onSuccess: () =>
      this.queryClient.invalidateQueries({
        queryKey: dispatcherQueryKeys.serviceDay(this.serviceDate()),
      }),
    onError: (error) => this.error.set(httpErrorMessage(error, 'Unable to import this Manifest.')),
  }));
  ...
  protected countInactive(): number {
    return (
      this.preview()?.rows.filter((row) => !row.isActive && row.disposition !== 'Blocked').length ??
      0
    );
  }
}
