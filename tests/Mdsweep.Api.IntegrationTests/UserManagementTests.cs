...
{
    private static readonly JsonSerializerOptions Json = new JsonSerializerOptions(JsonSerializerDefaults.Web).ConfigureForNodaTime(DateTimeZoneProviders.Tzdb);
    private const string TenantId = "mdsw-eep2-3456";
    private TestKeycloakUserAdministration Identity => Application.Services.GetRequiredService<TestKeycloakUserAdministration>();

    [Fact]
    public async Task Concurrent_invitations_do_not_duplicate_the_recipient()
    {
        using var manager = await Client();
        var request = new { email = "driver@example.test", firstName = "Synthetic", lastName = "Driver", roles = new[] { "Driver" } };
        var responses = await Task.WhenAll(manager.PostAsJsonAsync("/api/users/invitations", request), manager.PostAsJsonAsync("/api/users/invitations", request));
        Assert.Single(responses, x => x.StatusCode == HttpStatusCode.Created);
        Assert.All(responses, x => Assert.Contains(x.StatusCode, new[] { HttpStatusCode.Created, HttpStatusCode.Conflict, HttpStatusCode.BadRequest }));
    ...
    public async Task Email_failure_is_visible_and_retryable_without_duplicate_invitations()
    {
        ...
        var duplicate = await manager.PostAsJsonAsync("/api/users/invitations", new
        { email = "DRIVER@example.test", firstName = "Synthetic", lastName = "Driver", roles = new[] { "Driver" } });
        Assert.Equal(HttpStatusCode.BadRequest, duplicate.StatusCode);
        Identity.FailEmail = false;
        var sent = await (await manager.PostAsync($"/api/users/invitations/{invitation.Id}/resend", null)).Content.ReadFromJsonAsync<InvitationModel>(Json);
        Assert.NotNull(sent!.SentAt);
        Assert.Null(sent.DeliveryError);
        var history = await manager.GetFromJsonAsync<HistoryModel[]>($"/api/users/invitations/{invitation.Id}/history", Json);
    ...
    public async Task Unavailable_invitation_or_unverified_identity_cannot_gain_access(string scenario)
    {
        using var manager = await Client();
        var invitation = await Invite(manager);
        if (scenario == "revoked") (await manager.PostAsync($"/api/users/invitations/{invitation.Id}/revoke", null)).EnsureSuccessStatusCode();
        if (scenario == "expired")
        {
            await using var scope = Application.Services.CreateAsyncScope();
            var db = scope.ServiceProvider.GetRequiredService<ApplicationDbContext>();
            await db.Invitations.ExecuteUpdateAsync(x => x.SetProperty(i => i.ExpiresAt, NodaTime.SystemClock.Instance.GetCurrentInstant() - Duration.FromDays(1)));
        }
        if (scenario == "wrong-email") Identity.Email = "someone-else@example.test";
        if (scenario == "unverified") Identity.Verified = false;
        if (scenario == "not-member") Identity.IsMember = false;
        using var recipient = await Client("invited-subject");
        Assert.Equal(HttpStatusCode.BadRequest, (await recipient.PostAsync($"/api/invitation/{invitation.Id}/accept", null)).StatusCode);
        await using var scope2 = Application.Services.CreateAsyncScope();
    ...
    public async Task Dispatcher_cannot_manage_privileged_Users_or_grant_privileged_roles()
    {
        ...
        {
            var response = await manager.PostAsJsonAsync("/api/users/invitations", new { email = "new@example.test", firstName = "New", lastName = "User", roles = new[] { role } });
            Assert.Equal(HttpStatusCode.Forbidden, response.StatusCode);
        }
        Assert.Equal(HttpStatusCode.Forbidden, (await Update(manager, privileged, "Driver")).StatusCode);
        Assert.Equal(HttpStatusCode.Forbidden, (await manager.PostAsync($"/api/users/{privileged.Id}/password-reset", null)).StatusCode);
        Assert.Equal(HttpStatusCode.Forbidden, (await manager.GetAsync($"/api/users/{privileged.Id}/history")).StatusCode);
        Assert.DoesNotContain((await manager.GetFromJsonAsync<UserManagementModel>("/api/users", Json))!.Users, x => x.Id == privileged.Id);
        ...
        Assert.Equal(HttpStatusCode.Forbidden, (await manager.PostAsync($"/api/users/invitations/{privilegedInvitation.Id}/resend", null)).StatusCode);
        Assert.Equal(HttpStatusCode.Forbidden, (await manager.PostAsync($"/api/users/invitations/{privilegedInvitation.Id}/revoke", null)).StatusCode);
    }
    ...
    public async Task Administrator_can_edit_roles_and_names_and_deactivate_existing_sessions()
    {
        ...
        var updated = (await manager.GetFromJsonAsync<UserManagementModel>("/api/users", Json))!.Users.Single(x => x.Id == driver.Id);
        ...
        Assert.Empty((await target.GetFromJsonAsync<object[]>("/api/auth/me"))!);
        Assert.Equal(HttpStatusCode.Conflict, (await Update(manager, updated, "Driver")).StatusCode);
        var deactivated = (await manager.GetFromJsonAsync<UserManagementModel>("/api/users", Json))!.Users.Single(x => x.Id == driver.Id);
    ...
    public async Task Tenant_boundaries_are_enforced_for_reads_mutations_and_acceptance()
    {
        ...
        Assert.Equal(HttpStatusCode.NotFound, (await local.PostAsync($"/api/users/invitations/{invitation.Id}/revoke", null)).StatusCode);
        Assert.Equal(HttpStatusCode.NotFound, (await local.GetAsync($"/api/users/{outsider.Id}/history")).StatusCode);
        // A verified identity already bound locally cannot join another Tenant.
        Assert.Equal(HttpStatusCode.BadRequest, (await local.PostAsync($"/api/invitation/{invitation.Id}/accept", null)).StatusCode);
    ...
    public async Task Password_reset_reports_delivery_failures_and_retains_successful_requests()
    {
        ...
        using var manager = await Client();
        Identity.FailEmail = true;
        Assert.Equal(HttpStatusCode.BadRequest, (await manager.PostAsync($"/api/users/{driver.Id}/password-reset", null)).StatusCode);
        Identity.FailEmail = false;
    ...
    public async Task Anonymous_requests_and_missing_antiforgery_are_rejected()
    {
        ...
        var response = await manager.PostAsJsonAsync("/api/users/invitations", new
        { email = "driver@example.test", firstName = "Synthetic", lastName = "Driver", roles = new[] { "Driver" } });
    ...
    public async Task Two_role_invitation_creates_one_membership_and_grants_both_roles(string first, string second)
    {
        ...
        using var recipient = await Client("invited-subject");
        Assert.Equal(roles, (await recipient.GetFromJsonAsync<PendingInvitationModel>("/api/invitation", Json))!.Roles);
        (await recipient.PostAsync($"/api/invitation/{invitation.Id}/accept", null)).EnsureSuccessStatusCode();
        (await recipient.PostAsync($"/api/invitation/{invitation.Id}/accept", null)).EnsureSuccessStatusCode();
        var memberships = await recipient.GetFromJsonAsync<JsonElement[]>("/api/auth/me");
        Assert.Equal(roles.Order(), memberships!.Select(x => x.GetProperty("role").GetString()).Order());
        Assert.All(memberships, x => Assert.Equal(TenantId, x.GetProperty("tenantId").GetString()));
        Assert.Equal(HttpStatusCode.NoContent, (await recipient.PostAsJsonAsync("/api/auth/tenant-context", new { tenantId = TenantId })).StatusCode);
        Assert.Equal(HttpStatusCode.OK, (await recipient.GetAsync("/api/users")).StatusCode);
        Assert.Equal(HttpStatusCode.Forbidden, (await recipient.PostAsJsonAsync("/api/auth/tenant-context", new { tenantId = "abcd-efgh-jkmn" })).StatusCode);
        await using var scope = Application.Services.CreateAsyncScope();
        var db = scope.ServiceProvider.GetRequiredService<ApplicationDbContext>();
        var user = await db.Users.SingleAsync(x => x.KeycloakUserId == "invited-subject");
    ...
    public async Task Dispatcher_cannot_manage_Drivers_with_a_privileged_second_role(string privilegedRole)
    {
        ...
        Assert.Equal(HttpStatusCode.Forbidden, (await manager.PostAsync($"/api/users/{target.Id}/password-reset", null)).StatusCode);
        Assert.Equal(HttpStatusCode.Forbidden, (await manager.GetAsync($"/api/users/{target.Id}/history")).StatusCode);
        Assert.Equal(HttpStatusCode.Forbidden, (await manager.GetAsync($"/api/users/invitations/{invitation.Id}/history")).StatusCode);
        Assert.Equal(HttpStatusCode.Forbidden, (await manager.PostAsync($"/api/users/invitations/{invitation.Id}/resend", null)).StatusCode);
        Assert.Equal(HttpStatusCode.Forbidden, (await manager.PostAsync($"/api/users/invitations/{invitation.Id}/revoke", null)).StatusCode);
        var driver = await AddUser("Driver", "driver-subject", "driver2@example.test");
        Assert.Equal(HttpStatusCode.Forbidden, (await Update(manager, driver, roles)).StatusCode);
        Assert.Equal(HttpStatusCode.Forbidden, (await manager.PostAsJsonAsync("/api/users/invitations",
            new { email = "another@example.test", firstName = "Synthetic", lastName = "Invitee", roles })).StatusCode);
    }
    ...
    public async Task Role_changes_take_effect_immediately_preserve_history_and_ignore_order()
    {
        ...
        var reordered = (await manager.GetFromJsonAsync<UserManagementModel>("/api/users", Json))!.Users.Single(x => x.Id == driver.Id);
        ...
        Assert.Equal(HttpStatusCode.Forbidden, (await recipient.GetAsync("/api/users")).StatusCode);
        Assert.Single((await recipient.GetFromJsonAsync<JsonElement[]>("/api/auth/me"))!);
        Assert.Equal(HttpStatusCode.BadRequest, (await Update(manager, admin, new[] { "Dispatcher", "Driver" })).StatusCode);
    ...
    public async Task Invalid_role_sets_are_rejected_for_invites_and_edits(string rolesJson)
    {
        ...
        var payload = new { email = "driver@example.test", firstName = "Synthetic", lastName = "User", roles, isActive = true, version = admin.Version };
        Assert.Equal(HttpStatusCode.BadRequest, (await manager.PostAsJsonAsync("/api/users/invitations", payload)).StatusCode);
        Assert.Equal(HttpStatusCode.BadRequest, (await manager.PutAsJsonAsync($"/api/users/{admin.Id}", payload)).StatusCode);
        await using var scope = Application.Services.CreateAsyncScope();
        var db = scope.ServiceProvider.GetRequiredService<ApplicationDbContext>();
        Assert.Empty(await db.Invitations.ToListAsync());
        Assert.Equal(new[] { "Administrator" }, (await db.TenantMemberships.SingleAsync(x => x.UserId == admin.Id)).Roles);
    }
    ...
    private static Task<InvitationModel> Invite(HttpClient client, string role = "Driver", string email = "driver@example.test")
        => Invite(client, [role], email);

    private static async Task<InvitationModel> Invite(HttpClient client, string[] roles, string email = "driver@example.test")
    {
        var response = await client.PostAsJsonAsync("/api/users/invitations", new { email, firstName = "Synthetic", lastName = "Invitee", roles });
    ...
    private async Task<UserModel> AddUser(string[] roles, string subject, string email, string tenantId = TenantId)
    {
        ...
        if (!await db.Tenants.AnyAsync(x => x.Id == tenantId))
        {
            ...
            await scope.ServiceProvider.GetRequiredService<IDynamicTenantSource<string>>().AddTenantAsync(tenantId, CancellationToken.None);
        }
        var user = UserAggregate.Create("Synthetic", "User", subject, tenantId, email);
        db.Users.Add(user);
        db.TenantMemberships.Add(TenantMembership.Create(tenantId, user.Id, roles));
        await db.SaveChangesAsync();
        return new UserModel(user.Id, user.FirstName, user.LastName, email, roles, true, user.Version);
    }

    private static Task<HttpResponseMessage> Update(HttpClient client, UserModel user, string role, bool active = true, string firstName = "Synthetic")
        => Update(client, user, [role], active, firstName);

    private static Task<HttpResponseMessage> Update(HttpClient client, UserModel user, string[] roles, bool active = true, string firstName = "Synthetic")
        => client.PutAsJsonAsync($"/api/users/{user.Id}", new { firstName, lastName = "User", roles, isActive = active, version = user.Version });
}
